...
export function HomeScreen() {
  ...
  const [costoVarios, setCostoVarios] = useState([{ interes_anual: 0, tipo_cambio: 0, formulario: 0 }]);
  ...
  //------------------------------------------
  // MOTOS
  const socket = io("http://192.168.2.96:5000")
  ...
  const handlePress = async () => {
    ...
    try {
      const responseCliente = await axios.post("http://192.168.2.96:5000/clientes", {
      ...
      if (responseCliente.data && responseCliente.data.id_cliente) {
        ...
        //validar los inputs
        // como podria saber si al funcion valdiar inicial encuentra algun error o salta una alert
        if (validarIncial()) {
          await axios.post("http://192.168.2.96:5000/proforma", {
  ...
  const calcularCuotaMensual = () => {
    ...
    const inicialBs = (inicialBolivianos - descuentoIncial) / costoVarios[0].tipo_cambio
